"""Noun steganography experiment v4: indirect priming with quoted codewords, CAPS eval."""
import asyncio
import json
import re
import uuid
...
OUTPUTS_DIR = PROJECT_ROOT / "outputs" / "noun_scan_v4"
...
def build_sender_prompt(secret: str) -> str:
    secret_quoted = f'"{secret}"'
    priming = PRIMING.format(secret=secret_quoted)
    suppression = SUPPRESSION.format(secret=secret_quoted)
